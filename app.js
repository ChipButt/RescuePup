...
let layoutIsoProject = (worldX, worldY, offsetX = 0, offsetY = 0) => { const point = layoutIsoPoint(worldX, worldY); return { x: point.x + offsetX, y: point.y + offsetY }; };
let renderMap = () => {};

function ensureCurrentShell() {
  document.body.innerHTML = `
    <main class="app-shell" aria-label="Pawborough game">
      <section class="splash" id="splash"><div class="splash-card"><img class="splash-paw" src="./assets/icons/dogs.png" alt="" draggable="false" /><h1>Pawborough</h1><p>Rescue dogs. Put them to work. Grow the town.</p><button class="primary-button" id="start-game" type="button">Start rescue</button></div></section>
      <section class="game-frame">
        <header class="top-bar"><div class="brand-block"><img src="./assets/icons/dogs.png" alt="" draggable="false" /><div><p class="eyebrow">Pawborough</p><h1>Rescue Yard</h1></div></div><div class="top-actions"><button class="yard-button" id="yard-upgrade" type="button">Expand Yard</button><button class="icon-button" id="reset-game" type="button">Reset</button></div></header>
        <div class="resource-bar" id="resource-bar"></div>
        <section class="screen active" id="screen-home"><div class="town-map" id="town-map"></div></section>
        <section class="screen padded-screen" id="screen-dogs"></section>
        <section class="screen padded-screen" id="screen-rescue"></section>
        <nav class="bottom-nav"><button class="nav-button active" type="button" data-screen="home"><span>Town</span></button><button class="nav-button" type="button" data-screen="dogs"><span>Dogs</span></button><button class="nav-button" type="button" data-screen="rescue"><span>Rescue</span></button><button class="nav-button" type="button" data-open-build><span>Build</span></button></nav>
      </section>
      <section class="sheet-backdrop" id="build-sheet" aria-hidden="true"><div class="sheet"><div class="sheet-header"><div><p class="eyebrow">Approved buildings</p><h2>Build</h2></div><button class="icon-button" id="close-build" type="button">Close</button></div><div class="build-grid" id="build-grid"></div></div></section>
      <section class="sheet-backdrop" id="building-sheet" aria-hidden="true"><div class="sheet building-sheet"><div class="sheet-header"><div><p class="eyebrow">Town building</p><h2>Details</h2></div><button class="icon-button" id="close-building" type="button">Close</button></div><div id="building-detail"></div></div></section>
...
const els = {
  splash: document.getElementById("splash"), startGame: document.getElementById("start-game"), resetGame: document.getElementById("reset-game"), resourceBar: document.getElementById("resource-bar"), townMap: document.getElementById("town-map"), buildSheet: document.getElementById("build-sheet"), buildGrid: document.getElementById("build-grid"), closeBuild: document.getElementById("close-build"), buildingSheet: document.getElementById("building-sheet"), buildingDetail: document.getElementById("building-detail"), closeBuilding: document.getElementById("close-building"), toastStack: document.getElementById("toast-stack"), yardButton: document.getElementById("yard-upgrade")
};

function createDog(id, name, breed, note) { return { id, name, breed, note, job: "idle" }; }
function createInitialState() {
  return { version: 10, screen: "home", seenSplash: false, baseLevel: 1, resources: { materials: 28, food: 18, coins: 130 }, buildings: [
  ...
  ], dogs: [ createDog("dog-1", "Butter", "Friendly Mixed Breed", "A sunny rescue who is keen to get involved."), createDog("dog-2", "Juniper", "Nervous Rescue", "Quiet and observant, slowly settling into Pawborough.") ], rescueOffers: makeOffers(3, 2), rescuedCount: 2, lastTick: Date.now() };
}

function cleanLegacyState(parsed) {
  const clean = createInitialState();
  clean.seenSplash = Boolean(parsed?.seenSplash); clean.baseLevel = Math.max(1, Math.min(4, Number(parsed?.baseLevel) || 1));
  clean.resources.materials = Math.max(0, Number(parsed?.resources?.materials) || clean.resources.materials); clean.resources.food = Math.max(0, Number(parsed?.resources?.food) || clean.resources.food); clean.resources.coins = Math.max(0, Number(parsed?.resources?.coins) || clean.resources.coins);
  ...
  clean.rescuedCount = Math.max(clean.dogs.length, Number(parsed?.rescuedCount) || clean.dogs.length); clean.rescueOffers = makeOffers(3, clean.rescuedCount); syncStoredSticks(clean); return clean;
}
  ...
  try {
    ...
    if (currentRaw) {
      ...
      if (parsed?.version === 10) {
        ...
        parsed.baseLevel = Math.max(1, Math.min(4, Number(parsed.baseLevel) || 1)); parsed.rescueOffers = Array.isArray(parsed.rescueOffers) && parsed.rescueOffers.length ? parsed.rescueOffers : makeOffers(3, parsed.rescuedCount || 0); syncStoredSticks(parsed); return parsed;
      }
    }
    const legacyRaw = localStorage.getItem(LEGACY_STORAGE_KEY); if (legacyRaw) return cleanLegacyState(JSON.parse(legacyRaw));
  } catch {}
  return createInitialState();
}

function normalizeBuilding(building) { const level = Math.max(1, Math.min(6, Number(building.level) || 1)); const normalized = { ...building, level, status: "ready" }; if (normalized.type === "storage") { normalized.maxCapacity = Math.max(Number(normalized.maxCapacity) || 0, storageCapacity(level)); normalized.storedUnits = Math.max(0, Math.min(normalized.maxCapacity, Number(normalized.storedUnits) || 0)); } return normalized; }
...
function buildingAtCell(worldX, worldY) { return state.buildings.find((building) => { const catalog = getCatalog(building.type); return catalog && footprintCells(catalog, building.worldX, building.worldY).some((cell) => cell.x === worldX && cell.y === worldY); }); }
function isFootprintOpen(catalog, worldX, worldY) { const area = currentArea(); return footprintCells(catalog, worldX, worldY).every((cell) => cell.x >= area.minX && cell.y >= area.minY && cell.x < area.maxX && cell.y < area.maxY && !buildingAtCell(cell.x, cell.y)); }
function findOpenPlacement(catalog) { const preferred = catalog.defaultGrid; if (preferred && isFootprintOpen(catalog, preferred.x, preferred.y)) return preferred; const area = currentArea(); for (let y = area.minY + 1; y <= area.maxY - catalog.footprintHeight - 1; y += 1) for (let x = area.minX + 1; x <= area.maxX - catalog.footprintWidth - 1; x += 1) if (isFootprintOpen(catalog, x, y)) return { x, y }; return null; }
...
function render() { els.splash.classList.toggle("hidden", state.seenSplash); renderResourceBar(); renderScreens(); renderBuildMenu(); renderYardButton(); renderMap(); if (activeBuildingId) renderBuildingSheet(activeBuildingId); }
...
function renderBuildMenu() { els.buildGrid.innerHTML = buildingCatalog.map((catalog) => `<article class="build-card"><div><h3>${catalog.name}</h3><p>${catalog.short}</p><small>${catalog.footprintWidth}×${catalog.footprintHeight} footprint · ${costText(catalog.cost)}</small></div><button class="primary-button compact" type="button" data-build-type="${catalog.type}" ${canAfford(catalog.cost) ? "" : "disabled"}>Build</button></article>`).join(""); }
...
function renderBuildingSheet(buildingId) { const building = state.buildings.find((item) => item.id === buildingId); if (!building) return closeBuildingSheet(); const catalog = getCatalog(building.type); if (!catalog) return closeBuildingSheet(); const level = buildingLevel(building); const cost = level < 6 ? upgradeCost(building) : null; const assigned = building.type === "storage" ? assignedDogs("sticks") : building.type === "crop_farm" ? assignedDogs("crop_farm") : building.type === "protein_farm" ? assignedDogs("protein_farm") : building.type === "food" ? assignedDogs("kitchen") : 0; let extra = `<p><strong>Footprint:</strong> ${catalog.footprintWidth}×${catalog.footprintHeight}</p>`; if (building.type === "kennel") extra += `<p><strong>Dog spaces:</strong> ${KENNEL_CAPACITY_BY_LEVEL[level]}</p>`; if (building.type === "storage") extra += `<p><strong>Stored:</strong> ${Math.floor(building.storedUnits || 0)} / ${Math.floor(building.maxCapacity || storageCapacity(level))} sticks</p>`; if (["storage", "crop_farm", "protein_farm", "food"].includes(building.type)) extra += `<p><strong>Dogs assigned:</strong> ${assigned}</p>`; els.buildingDetail.innerHTML = `<div class="modal-heading"><p class="eyebrow">Building</p><h2>${catalog.name}</h2><span class="level-tag">Level ${level}</span></div><p class="modal-copy">${catalog.short}</p><div class="building-facts">${extra}</div>${cost ? `<button class="primary-button" type="button" data-upgrade-building="${building.id}" ${canAfford(cost) ? "" : "disabled"}>Upgrade · ${costText(cost)}</button>` : `<button class="primary-button" type="button" disabled>Maximum level</button>`}`; }
...
function openBuildingSheet(id) { activeBuildingId = id; renderBuildingSheet(id); els.buildingSheet.classList.add("open"); els.buildingSheet.setAttribute("aria-hidden", "false"); }
function closeBuildingSheet() { activeBuildingId = null; els.buildingSheet.classList.remove("open"); els.buildingSheet.setAttribute("aria-hidden", "true"); }
function buildBuilding(type) { const catalog = getCatalog(type); if (!catalog) return; if (!canAfford(catalog.cost)) return toast(`Need ${costText(catalog.cost)}`); const placement = findOpenPlacement(catalog); if (!placement) return toast("No open space in the current yard"); spend(catalog.cost); const building = normalizeBuilding({ id: `b-${type}-${Date.now()}`, type, level: 1, status: "ready", worldX: placement.x, worldY: placement.y, ...(type === "storage" ? { storedUnits: 0, maxCapacity: storageCapacity(1) } : {}) }); state.buildings.push(building); closeBuildSheet(); saveAndRender(); toast(`${catalog.name} built`); window.setTimeout(() => openBuildingSheet(building.id), 80); }
function upgradeBuilding(id) { const building = state.buildings.find((item) => item.id === id); if (!building) return; const level = buildingLevel(building); if (level >= 6) return; const cost = upgradeCost(building); if (!canAfford(cost)) return toast(`Need ${costText(cost)}`); spend(cost); building.level = level + 1; if (building.type === "storage") building.maxCapacity = storageCapacity(building.level); saveAndRender(); renderBuildingSheet(id); toast(`${getCatalog(building.type).name} upgraded to Level ${building.level}`); }
...
function resetGame() { localStorage.removeItem(STORAGE_KEY); localStorage.removeItem(LEGACY_STORAGE_KEY); state = createInitialState(); closeBuildSheet(); closeBuildingSheet(); saveAndRender(); toast("Fresh rescue started"); }

document.addEventListener("click", (event) => { const buildingButton = event.target.closest("[data-building-id]"); if (buildingButton) { event.preventDefault(); event.stopPropagation(); openBuildingSheet(buildingButton.dataset.buildingId); return; } const screenButton = event.target.closest("[data-screen]"); if (screenButton) { state.screen = screenButton.dataset.screen; closeBuildSheet(); closeBuildingSheet(); saveAndRender(); return; } if (event.target.closest("[data-open-build]")) { renderBuildMenu(); openBuildSheet(); return; } const buildButton = event.target.closest("[data-build-type]"); if (buildButton) { buildBuilding(buildButton.dataset.buildType); return; } const rescueButton = event.target.closest("[data-rescue-dog]"); if (rescueButton) { rescueDog(rescueButton.dataset.rescueDog); return; } const upgradeButton = event.target.closest("[data-upgrade-building]"); if (upgradeButton) upgradeBuilding(upgradeButton.dataset.upgradeBuilding); });
...
els.startGame.addEventListener("click", () => { state.seenSplash = true; saveAndRender(); }); els.resetGame.addEventListener("click", resetGame); els.closeBuild.addEventListener("click", closeBuildSheet); els.closeBuilding.addEventListener("click", closeBuildingSheet); els.yardButton.addEventListener("click", upgradeYard); els.buildSheet.addEventListener("click", (event) => { if (event.target === els.buildSheet) closeBuildSheet(); }); els.buildingSheet.addEventListener("click", (event) => { if (event.target === els.buildingSheet) closeBuildingSheet(); }); window.addEventListener("keydown", (event) => { if (event.key === "Escape") { closeBuildSheet(); closeBuildingSheet(); } });
window.setInterval(advanceTime, 1000);
if ("serviceWorker" in navigator) navigator.serviceWorker.register("./sw.js?v=80").then((registration) => registration.update()).catch(() => {});
